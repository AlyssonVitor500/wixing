<?php
    ...
    if ($select == 1) {
                ...
                $sql = mysqli_query($conn, "SELECT * FROM logs as l,  produtos as p, fornecedor as f WHERE l.prodIdFK = p.id and p.idFornFK = f.idForn and l.dia >= '$de' and l.dia <= '$ate' and $tipo = 2 ORDER BY l.dia DESC");
                while ($dados = $sql->fetch_assoc()) {
                    $idLog = $dados['idLog'];
                    $tipoLog = $dados['tipo'];
                    $quant = $dados['quant_prod'];
